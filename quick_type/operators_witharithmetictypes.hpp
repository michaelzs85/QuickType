...
typename std::enable_if<conjunction<std::is_base_of<AdditionWithArith, NamedType>, std::is_arithmetic<T>>::value, NamedType>::type
...
template <class NamedType, typename T>
typename std::enable_if<conjunction<std::is_base_of<BitwiseRightShiftAssignmentWithArith, NamedType>, std::is_arithmetic<T>>::value,
                        NamedType &>::type
operator>>=(NamedType &lhs, const T &rhs)
{
    lhs.val >>= rhs;
